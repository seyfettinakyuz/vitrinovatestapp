...
import com.seyfettin.vitrinovaapplication.databinding.ItemEditorBinding;
...
public class EditorShopListAdapter extends BaseAdapter<EditorShopListAdapter.ViewHolder, EditorShop> {

    private List<EditorShop> data;
    private int lastPosition = -1;


    public EditorShopListAdapter() {
        data = new ArrayList<>();
    }
    ...
    static class ViewHolder extends RecyclerView.ViewHolder {
        private static ViewHolder create(LayoutInflater inflater, ViewGroup parent) {
            ItemEditorBinding itemBinding = ItemEditorBinding.inflate(inflater, parent, false);
            return new ViewHolder(itemBinding);
        }

        final ItemEditorBinding binding;

        private ViewHolder(ItemEditorBinding binding) {
            super(binding.getRoot());
            this.binding = binding;
        }

        private void onBind(EditorShop editorShop) {
            binding.setEditor(editorShop);
